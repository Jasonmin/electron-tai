...
// import { app } from "electron"; // elec-zq


// ==================================================  taiji  ===================================================

var http_code_key = window["__sealtalk_config"]["http_code_key"];
console.log(`http_code_key::${http_code_key}`);
    ...
    function($scope, $state, mainServer, mainDataServer, conversationServer, RongIMSDKServer) {
        ...
        $scope.signin = function() {
            ...
            if ($scope.formSignin.$valid) {
                mainServer.user.tj_signin($scope.user.accountNumber, "86", $scope.user.passWord).success(function(rep) {
                    if (rep[http_code_key] === 200) {
                        console.log(`登录成功`)
                        console.log(`res::${JSON.stringify(rep)}`)
                        mainDataServer.loginUser.id = rep.data.id;
                        mainDataServer.loginUser.token = rep.data.rytoken;
                        mainDataServer.loginUser.authtoken = rep.data.authtoken;
                        mainDataServer.loginUser.username = rep.data.username;
                        mainDataServer.loginUser.nickName = rep.data.name;
                        mainDataServer.loginUser.name = rep.data.name;
                        mainDataServer.loginUser.portraitUri = rep.data.portraituri;
                        var exdate = new Date();
                        exdate.setDate(exdate.getDate() + 30);
                        webimutil.CookieHelper.setCookie("loginuserid", mainDataServer.loginUser.id, exdate.toGMTString());
                        webimutil.CookieHelper.setCookie("loginusertoken", mainDataServer.loginUser.token, exdate.toGMTString());
                        localStorage.setItem(`authtoken`, mainDataServer.loginUser.authtoken)
                        localStorage.setItem(`username`, mainDataServer.loginUser.username)
                        $state.go("main");
...
conversationCtr.controller("conversationController", ["$scope", "$state", "mainDataServer", "conversationServer", "mainServer", "RongIMSDKServer", "$http", "$timeout", "$location", "$anchorScroll",
    function($scope, $state, mainDataServer, conversationServer, mainServer, RongIMSDKServer, $http, $timeout, $location, $anchorScroll) {
        ...
        RongIMSDKServer.clearUnreadCount(targetType, targetId); //zq
        ...
        $scope.sendReadReceiptRequestMessage = function(uid) {
            ...
            RongIMSDKServer.sendMessage(targetType, targetId, reqMsg).then(function() {}, function(error) {
        ...
        function sendReadReceiptResponseMessage() {
            ...
            RongIMSDKServer.sendReceiptResponse(targetType, targetId).then(function() {}, function(error) {
        ...
        $scope.sendTypingStatusMessage = function() {
            ...
            RongIMSDKServer.sendMessage(targetType, targetId, msg).then(function() {}, function(error) {
        ...
        function updateTargetDetail() {
            if (targetType == webimmodel.conversationType.Private) {
                ...
                if (friend) {
                    mainServer.friend.tj_getInfo_getProfile(targetId).success(function(data) {
        ...
        RongIMLib.RongUploadLib.getInstance().setListeners({
            ...
            onFileUploaded: function(file, message) {
                if (file.uploadType == 'IMAGE') {
                    $scope.uploadStatus.show = false;
                    $scope.uploadStatus.progress = 0;
                } else {
                ...
                if (scope.atShow && (e.keyCode == 38 || e.keyCode == 40)) {
                    ...
                    if (selItem.length) {
                        ...
                        if (nextItem && nextItem.position()) {
                            var _target = $("#atList"),
                                _offsetTop = nextItem.position().top;
                            if (nextItem.index() == 1) {
                                _scrollTop = 0;
                            }
            ...
            switch (msg.messageType) {
                ...
                default:
                    if (SDKmsg.objectName == "ST:GrpNtf") {
                        ...
                        msg.content = groupnot;
                        msg.panelType = webimmodel.PanelType.InformationNotification;
                    } else if (SDKmsg.objectName == "RC:RLStart" || SDKmsg.objectName == "RC:RL" || SDKmsg.objectName == "RC:RcCmd") {} else {
                        msg.content = '当前版本暂不支持查看此消息';
                        msg.panelType = webimmodel.PanelType.InformationNotification;
                        console.log("has unknown message type " + SDKmsg.messageType);
                    }
...
conversationServer.factory("conversationServer", ["$q", "mainDataServer", "mainServer", "RongIMSDKServer",
    function($q, mainDataServer, mainServer, RongIMSDKServer) {
        ...
        function getHistory(id, type, lastTime, count) {
            ...
            try {
                RongIMSDKServer.getHistoryMessages(+conver, currentConversationTargetId, lastTime, count).then(function(data) {
                    ...
                    d.resolve(false); // ele-zq 不获取历史消息
                    // d.reject(err);
        ...
        function messageAddUserInfo(item) {
            ...
            if (item.messageDirection == 1) {
                ...
                if (user) {
                    ...
                    mainServer.user.tj_getInfo(item.senderUserId).success(function(rep) {
                        if (rep[http_code_key] == 200) {
                            item.senderUserName = rep.data.name;
                            item.senderUserImgSrc = webimutil.ChineseCharacter.getPortraitChar(rep.data.name);
                            item.imgSrc = rep.data.portraituri;
        ...
        function sendReadReceiptMessage(id, type, messageuid, sendtime) {
            ...
            RongIMSDKServer.sendMessage(type, id, msg).then(function() {}, function(error) {
        ...
        $scope.applyfriendbtn = function() {
            ...
            mainServer.friend.invite(id, $scope.message).success(function(rep) {
                ...
                if (action == "Added") {
                    ...
                    mainServer.user.tj_getInfo(id).success(function(rep) {
                        addfriend.imgSrc = rep.data.portraituri;
                    });
...
friendinfo.controller("editfriendinfoController", ["$scope", "$state", "$stateParams", "$window", "mainDataServer", "mainServer", "RongIMSDKServer",
    function($scope, $state, $stateParams, $window, mainDataServer, mainServer, RongIMSDKServer) {
        ...
        if (friend) {
            ...
            mainServer.user.tj_getInfo(userid).then(function(rep) {
                /**
                             * mainDataServer.loginUser.nickName = rep.data.name;
                mainDataServer.loginUser.firstchar = webimutil.ChineseCharacter.getPortraitChar(rep.data.name);
                mainDataServer.loginUser.portraitUri = rep.data.portraituri; */
                $scope.user.id = rep.data.result.id;
                $scope.user.nickName = rep.data.name;
                $scope.user.portraitUri = rep.data.portraituri;
                $scope.user.firstchar = webimutil.ChineseCharacter.getPortraitChar(rep.data.name);
        ...
        $scope.removeFriend = function() {
            if (loading)
                return;
            loading = true;
            mainServer.friend.setDisplayName($scope.user.id, "").success(function() {}).error(function() {
...
friendinfo.controller("friendinfoController", ["$scope", "$rootScope", "$state", "$stateParams", "$window", "mainDataServer", "mainServer",
    function($scope, $rootScope, $state, $stateParams, $window, mainDataServer, mainServer) {
        ...
        $scope.user = new webimmodel.UserInfo();
        if (friend) {
            mainServer.friend.tj_getInfo_getProfile(userid).success(function(rep) {
                var f = new webimmodel.Friend({ id: rep.data.id, name: rep.data.name, imgSrc: rep.data.portraituri });
                f.displayName = rep.data.name;
                f.mobile = rep.data.tel;
                f = mainDataServer.contactsList.updateOrAddFriend(f);
                mainDataServer.conversation.updateConversationDetail(webimmodel.conversationType.Private, userid, rep.data.name || rep.data.name, rep.data.portraituri);
                $scope.user.id = f.id;
...
var appcenter = angular.module("webim.appcenter", []);
appcenter.controller("appcenterController", ["$scope", "$state", "$stateParams", "mainDataServer", function($scope, $state, $stateParams, mainDataServer) {
    $scope.$on("$viewContentLoaded", function() {
        console.log(`appname:`)

        var appname = $stateParams['appname']
        var appurl = $stateParams['appurl']
        var tj_username = localStorage.getItem(`username`);

        console.log(`==>${appname},${appurl},${tj_username}`)
        ...
        webview.setAttribute('src', `${appurl}?username=${tj_username}`)
...
groupInfo.controller("groupbulletinController", ["$scope", "$state", "$stateParams", "mainServer", "mainDataServer", "RongIMSDKServer", "conversationServer",
    function($scope, $state, $stateParams, mainServer, mainDataServer, RongIMSDKServer, conversationServer) {
        var groupid = $stateParams["groupid"],
            targettype = RongIMLib.ConversationType.GROUP;
...
mainServer.factory("mainServer", ["$http", "$q", "appconfig", function($http, $q, appconfig) {
    ...
    var serverBaseIP = appconfig.getBaseIP();
    var h5AppUrl = appconfig.getH5AppUrl();
    var tj_headers = {
        'content-type': 'application/json',
        'authorization': localStorage.getItem(`authtoken`),
    };
    var tj_userId = localStorage.getItem(`loginuserid`);
    var tj_username = localStorage.getItem(`username`);
    var mainServer = {
        user: {
            ...
            tj_signin: function(phone, region, password) {
                return $http({
                    method: "POST",
                    url: serverBaseUrl + "/auth/jwt/login",
                    data: {
                        username: "taiji",
            ...
            tj_getInfo: function(id) {
                return $http({
                    method: "get",
                    headers: tj_headers,
                    url: serverBaseUrl + "/admin/userapi/" + id
                });
            },
            ...
            tj_getMyGroups: function() {
                return $http({
                    method: "POST",
                    headers: tj_headers,
                    url: serverBaseUrl + "/admin/ryapi/group/list/" + tj_userId
                });
            },
            tj_getApps: function() {
                return $http({
                    method: "POST",
                    headers: tj_headers,
                    url: h5AppUrl + "/AppApi/GetAppMenus",
                    data: {
                        userName: tj_username,
                        token: getWebTokenWithUserName(tj_username)
                    }
                });
            },
        ...
        friend: {
            ...
            tj_getAll: function() {
                return $http({
                    method: "get",
                    headers: tj_headers,
                    url: serverBaseUrl + "/admin/userapi/get/relation/" + tj_userId,
                });
            },
            ...
            tj_getInfo_getProfile: function(id) {
                return $http({
                    method: "get",
                    headers: tj_headers,
                    url: serverBaseUrl + "/admin/userapi/" + id
...
mainServer.factory("mainDataServer", ["$q", "RongIMSDKServer", "mainServer", function($q, RongIMSDKServer, mainServer) {
        ...
        parseConversation: function(item) {
            ...
            switch (item.conversationType) {
                ...
                case RongIMLib.ConversationType.GROUP:
                    ...
                    removeUnreadCount = 0;
                    if (!group) {
                        if (item.targetId == '__system__') {} else {
                        ...
                        if (conversationitem.lastMsg && item.latestMessage.objectName != "ST:GrpNtf" && item.latestMessage.objectName != "RC:InfoNtf") {
                            ...
                            if (member) {
                                ...
                                (function(id, conv) {
                                    mainServer.user.tj_getInfo(id).success(function(user) {
                                        conv.lastMsg = user.data.name + "：" + conversationitem.lastMsg;
                                    });
        ...
        updateConStatic: function(msg, add, isChat) {
            ...
            RongIMSDKServer.getConversation(type, id).then(function(data) {
                if (data) {
                    ...
                    var oldUnread = 0,
                        totalUnreadCount = mainDataServer.conversation.totalUnreadCount,
                        isfirst = false,
                        conversationItem;
                    for (var i = 0, len = mainDataServer.conversation.conversations.length; i < len; i++) {
    ...
    var appsList = [];
    mainDataServer.appsList = appsList;
    ...
    function($scope, $state, $window, $timeout, $http, mainDataServer, conversationServer, mainServer, RongIMSDKServer, appconfig) {
        ...
        if (!mainDataServer.loginUser.id) {
            var userid = webimutil.CookieHelper.getCookie("loginuserid"),
                usertoken = webimutil.CookieHelper.getCookie("loginusertoken");
            if (userid) {
        ...
        mainServer.user.tj_getInfo(mainDataServer.loginUser.id).success(function(rep) {
            if (rep[http_code_key] == 200) {
                mainDataServer.loginUser.nickName = rep.data.name;
                mainDataServer.loginUser.firstchar = webimutil.ChineseCharacter.getPortraitChar(rep.data.name);
                mainDataServer.loginUser.portraitUri = rep.data.portraituri;
        ...
        $scope.selectGoAppCenter = function(appname, appurl) {
            if ($scope.switchbtn.isAppCenter) {
                $state.go("main.appcenter", { appname: appname, appurl: appurl });
            } else {
                $state.go("main.chat", { targetId: id, targetType: type }, { location: "replace" });
            }
        };
        ...
        mainServer.friend.tj_getAll().success(function(rep) {
            var arr = rep.data;
            for (var i = 0, len = arr.length; i < len; i++) {
                mainDataServer.contactsList.addFriend(new webimmodel.Friend({
                    id: arr[i].id,
                    name: arr[i].name || arr[i].nickname,
                    imgSrc: arr[i].portraituri,
                }));
                // zq
                // switch (arr[i].status) {
        ...
        mainServer.user.tj_getMyGroups().success(function(rep) {
            if (rep[http_code_key] === 200) {
                console.log(`获取群组列表成功`)
                var groups = rep.data;
                for (var i = 0, len = groups.length; i < len; i++) {
                    var members = groups[i].membersid.split(`,`)
        ...
        var imappkey = appconfig.getAppKey();
        // zq
        var imRyToken = appconfig.getRyToken();
        mainDataServer.loginUser.token = imRyToken;

        // app center menus
        mainServer.user.tj_getApps().success(function(rep) {
            if (rep["code"] === 0) {
                ...
                for (var i = 0, len = apps.length; i < len; i++) {
                    ...
                    item.id = apps[i].id
                    item.imgSrc = apps[i].icon
                    item.url = apps[i].url
                        // item.firstchar = name[0]

                    mainDataServer.appsList.push(item);
                }
            } else {}
        }).error(function(err) {});

        RongIMSDKServer.init(imappkey);
                        ...
                        if (isself || $state.is("main.chat") && !document.hidden && msg.conversationType == mainDataServer.conversation.currentConversation.targetType && msg.senderUserId == mainDataServer.conversation.currentConversation.targetId) {
                            ...
                            if (msg.senderUserName) {
                                webimutil.NotificationHelper.showNotification({
                                    ...
                                    icon: "assets/img/SealTalk.ico",
                                    body: webimmodel.Message.messageToNotification(data, mainDataServer.loginUser.id, true),
                                    data: { targetId: msg.targetId, targetType: msg.conversationType }
                                });
                            } else {
...
var webimApp = angular.module("webim", ["ui.router", "ui.event", "uiSwitch", "ng.shims.placeholder",
    ...
    "webim.creategroup", "webim.addgroup", "webim.groupaddmember", "webim.goupinfo", "webim.appcenter",
    "webim.userinfo", "webim.blacklist", "webim.notification", "webim.usermodifypassword",
    "webim.account", "webim.creatediscussion", "webim.discussionaddmember", "webim.discussioninfo"
], function() {});
webimApp.config(["$provide", "$stateProvider", "$urlRouterProvider", "$httpProvider",
    function($provide, $stateProvider, $urlRouterProvider, $httpProvider) {
        var baseIP = window["__sealtalk_config"]["serverIP"];
        var baseUrl = window["__sealtalk_config"]["serverUrl"];
        var appkey = window["__sealtalk_config"]["appkey"];
        var ryToken = window["__sealtalk_config"]["ryToken"];
        var h5AppUrl = window["__sealtalk_config"]["h5AppUrl"];
        ...
        }]).when("/main/appcenter/:appname/:appurl", ["$state", "$match", "mainDataServer", function($state, $match, mainDataServer) {
            if (!mainDataServer.loginUser.nickName) {
                $state.go("main");
                return;
            } else {
                $state.transitionTo("main.appcenter", $match);
            }
        ...
        $stateProvider.state("forgetPassword", {
            ...
            url: "/appcenter/:appname/:appurl",
            templateUrl: "assets/views/appcenter.html",
...
mainDire.directive("appitem", ["$state", function($state) {
    return {
        restrict: "E",
        scope: { item: "=" },
        template: '<div class="app_item ">' +
            '<div class="photo">' +
            '<img class="img" ng-show="item.imgSrc" width="30" height="30" ng-src="{{item.imgSrc||\'assets/img/barBg.png\'}}" alt="">' +
            '<div class="portrait" ng-show="!item.imgSrc">{{item.firstchar}}</div>' +
            '</div>' +
            '<div class="info">' +
            '<h3 class="nickname">' +
        ...
        link: function(scope, ele, attr) {
            ...
            ele.on("click", function() {
                ...
                scope.$parent.selectGoAppCenter(scope.item.name, scope.item.url);
            });
        }
    };
}]);
mainDire.directive("frienditem", ["$state", function($state) {
    return {
        ...
        link: function(scope, ele, attr) {
            ...
            angular.element(ele[0].getElementsByClassName("portrait")[0]).css("background-color", webimutil.Helper.portraitColors[itemid.charCodeAt(0) % webimutil.Helper.portraitColors.length]);
            ele.on("click", function() {
